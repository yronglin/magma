    ...
    for( panelj=0; panelj < min_mn; panelj += nb)
    {
        ...
        for (step=0; step < ib; step++) {
            ...
            if ((m-panelj) > MAX_NTHREADS) {
                ...
                arginfo = magma_ihamax_batched(m-gbj, dA_array, ai+gbj, aj+gbj, ldda, 1, ipiv_array, ai+gbj, gbj, gbstep, info_array, batchCount, queue);
                if (arginfo != 0 ) return arginfo;
                // Apply the interchange to columns 1:N. swap the whole row
                arginfo = magma_hswap_batched(n, dA_array, ai, aj, ldda, gbj, ipiv_array, batchCount, queue);
                if (arginfo != 0 ) return arginfo;
                // Compute elements J+1:M of J-th column.
                if (gbj < m) {
                    arginfo = magma_hscal_hger_batched( m-gbj, ib-step, dA_array, ai+gbj, aj+gbj, ldda, info_array, gbj, gbstep, batchCount, queue );
                    if (arginfo != 0 ) return arginfo;
                }
            }
            else {
                arginfo = magma_hcomputecolumn_batched(m-panelj, panelj, step, dA_array, ai, aj, ldda, ipiv_array, info_array, gbstep, batchCount, queue);
                if (arginfo != 0 ) return arginfo;
                // Apply the interchange to columns 1:N. swap the whole row
        ...
        if ( (n-panelj-ib) > 0) {
            ...
            magma_hgetf2trsm_batched(ib, n-panelj-ib, dA_array, ai+panelj, ldda, batchCount, queue);
            // do the blocked DGER = DGEMM for the remaining panelj+ib:n columns
            magma_hgemm_batched_core( MagmaNoTrans, MagmaNoTrans, m-(panelj+ib), n-(panelj+ib), ib,
...
#define ipiv_array(i) ipiv_array, i
    ...
    magma_int_t arginfo = 0;
    if(n <= stop_nb){
        arginfo = magma_hgetf2_fused_batched(m, n, dA_array(ai,aj), ldda, ipiv_array, info_array, batchCount, queue);
    }
    else{
        magma_int_t n1, n2;
        n1 = n/2;
        ...
        // panel 1
        arginfo = magma_hgetf2_batched_v2(
                    m, n1, stop_nb,
                    dA_array(ai,aj), ldda,
    ...
    magma_queue_t queue)
{
    ...
    for(magma_int_t inb = 32; inb >= 2; inb/=2 ) {
        arginfo = magma_hgetf2_batched_v2(m, n, inb, dA_array, ai, aj, ldda, ipiv_array, dpivinfo_array, info_array, batchCount, queue);
        if(arginfo == 0) break;
    }

    // negative arginfo means that fused panel did not launch
    if( arginfo != 0 ) {
        arginfo = magma_hgetf2_batched_v1(m, n, dA_array, ai, aj, ldda, ipiv_array, info_array, gbstep, batchCount, queue);
    }
