...
#define PRECISION_s
magma_int_t magma_get_htrsm_batched_nb(magma_int_t n)
{
...
#define dB_array(i,j) dB_array, i, j
    ...
    magma_int_t batrsm_stop_nb = magma_get_htrsm_batched_stop_nb(side, m, n);
    // stopping condition
    if(nrowA <= batrsm_stop_nb){
        magmablas_htrsm_small_batched(side, uplo, transA, diag, m, n, alpha, dA_array(Ai, Aj), ldda, dB_array(Bi, Bj), lddb, batchCount, queue );
        return;
    }
    ...
    switch(shape)
    {
        case 0: // lNl
            {
                const int m2 = magma_get_htrsm_batched_nb(m); 
                const int m1 = m - m2;
        ...
        case 3: // lTU | lCU
            {
                const int m1 = magma_get_htrsm_batched_nb(m); 
                const int m2 = m - m1;
        ...
        case 4: // rNL
             {
                const int n2 = magma_get_htrsm_batched_nb(n); 
                const int n1 = n - n2;
        ...
        case 5: // rNU
            {
                const int n1 = magma_get_htrsm_batched_nb(n); 
                const int n2 = n - n1;
